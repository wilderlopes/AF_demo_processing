...
public class AF_demo_processing extends PApplet {
...
*/
          ...
          //Library to enable matrix and linear algebra calculations;
...
public void setup() {
  ...
  lms1 = new LMS(color(200,0,0), wo, regressor, var_v, graphX + 50, 600, 10, 0.005f);
  lms2 = new LMS(color(0,200,0), wo, regressor, var_v, graphX + 300, 300, 10, 0.01f);
  lms3 = new LMS(color(0,0,200), wo, regressor, var_v, graphX + 550, 500, 10, 0.05f);
...
class Target {      //For the system to be identified
  ...
  Target(int tempc, float tempxpos, float tempypos, float tempradius) {
  ...
  public void display() {         //Display circle
    stroke(255);
    fill(c);
...
class LMS {             //For each LMS AF
  ...
  LMS(int tempc, float[] tempwo, float[] tempuj, float tempvar_v, float tempxpos, float tempypos, float tempradius, float tempmu) {
    c = tempc;            //circle color
  ...
  public void addNoise() {       //Increases the variance of the measumerement noise;
    var_v_lms = var_v_lms*sqrt(10);    
  }  
  
  public void subNoise() {       //Decreases the variance of the measumerement noise;
    var_v_lms = var_v_lms/sqrt(10);    
  } 
    
  public void adapt() {          //adaptation loop
    d = 0;
    y = 0;   
...
public void keyPressed() {
  if (key == CODED) {
    ...
    if (keyCode == SHIFT) {      //Restart AFs from a random psosition;
      ...
      var_v = 1e-6f;
      var_q = 0;
      lms1 = new LMS(color(200,0,0), wo, regressor, var_v, random(graphX,width), random(text_y+15,height), 10, 0.005f);
      lms2 = new LMS(color(0,200,0), wo, regressor, var_v, random(graphX,width), random(text_y+15,height), 10, 0.01f);
      lms3 = new LMS(color(0,0,200), wo, regressor, var_v, random(graphX,width), random(text_y+15,height), 10, 0.05f);      
    }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "AF_demo_processing" };
    if (passedArgs != null) {
